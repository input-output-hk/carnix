    ...
    fn iter<'a>(&'a self) -> impl Iterator<Item = (&'a String, &'a (Crate, PathBuf))> {
        match *self {
            CrateType::Workspace {
                ...
                members
                    .iter()
                    .chain(package.iter().map(|&(ref a, ref b)| (a, b))),
            ),
            CrateType::Single(ref name, ref cr) => Either::B(std::iter::once((name, cr))),
        }
    }
}
...
fn workspace_members(base_path: &Path, cargo_toml: &toml::Value) -> Option<CrateType> {
    ...
    if let Some(ws) = cargo_toml.get("workspace") {
        ...
        let members = match ws.as_table().unwrap().get("members") {
        ...
        return Some(CrateType::Workspace {
...
pub fn generate_nix<W: Write, P: AsRef<Path>, Q: AsRef<Path>>(
    ...
    let patches = cargo_toml.get("patch").and_then(|x| {
        x.get("crates-io")
            .map(|x| make_deps(cr.parent().unwrap(), x))
    });
    ...
    let main_meta = {
        let root = if let Some(root) = lock.as_table().unwrap().get("root") {
            Some(root)
        } else if let Some(name) = cargo_toml
            .get("package")
            .and_then(|package| package.as_table().and_then(|package| package.get("name")))
        {
...
fn local_source_type<P: AsRef<Path>>(
    ...
    patches: Option<&BTreeMap<String, Dep>>,
    name: &str,
) -> SourceType {
    ...
    if let SourceType::None = loc {
        let mut dep = patches.and_then(|patches| patches.get(name));
        if dep.is_none() {
            dep = deps.get(name)
        };
...
fn output<W: Write>(
    ...
    if standalone {
        ...
        nix_file.write_all(
            b"{ lib, buildPlatform, buildRustCrate, buildRustCrateHelpers, cratesIO, fetchgit }:\n",
        )?;
    }
    ...
    for (_, &(ref cra, _)) in workspace_members.iter() {
        ...
        let version = format!("{}.{}.{}{}", cra.major, cra.minor, cra.patch, cra.subpatch);
        writeln!(
            nix_file,
            "  {} = crates.crates.{}.\"{}\" deps;",
            name, name, version,
...
impl Crate {
    pub fn output_package_call<W: Write>(
        ...
        write!(w, "{}deps.{}.\"{}\" = {{", indent, nix_name_, version)?;
        let mut is_first = true;
    ...
    pub fn output_package_features<W: Write>(
        ...
        let indent = std::iter::repeat(' ').take(n_indent).collect::<String>();
        let version = format!(
            "{}.{}.{}{}",
            self.major, self.minor, self.patch, self.subpatch
        );
        ...
        for (_, dep) in meta.dependencies.iter()
            .chain(meta.build_dependencies.iter())
            .chain(meta.target_dependencies.iter().flat_map(|&(_, ref y)| y.iter()))
            .filter(|&(_, dep)| dep.cr.found_in_lock)
        {
            debug!("outputting dep = {:?}", dep);
            let dep_name = format!(
                "{}.\"${{deps.{}.\"{}\".{}}}\"",
            ...
            let optional_dep_name = |feat, val| if dep.is_optional {
                Some(format!(
                    "(f: if deps.\"{}\".\"{}\" ? \"{}\" then recursiveUpdate f {{ {}.\"${{deps.\"{}\".\"{}\".\"{}\"}}\".\"{}\" = {}; }} else f)",
                    // if
                    nix_name_,
                    version,
                    nix_name(&dep.cr.name),
            ...
            for feat in dep.features.iter() {
                let mut e = output_features
                    .entry(format!("{}.\"{}\"", dep_name, feat))
                    .or_insert(Vec::new());
            ...
            if !dep.conditional_features.is_empty() {
                for feat in dep.conditional_features.iter() {
                    if !seen.contains(&(&dep.cr.name, &feat.feature)) {
                        let dep_name =
                            format!("{}.\"{}\"", dep_name, FeatName(&feat.dep_feature),);
            ...
            if !dep.default_features {
                if default.get(&dep_name).is_none() {
                    default.insert(dep_name, (optional_dep_name("default", false), false));
                }
            } else {
                default.insert(dep_name, (optional_dep_name("default", true), true));
            }
        }
        for (dep_name, (optional, default)) in default.iter() {
            if optional.is_none() {
                // We'll take care of optional dependencies later.
        ...
        for (_, (optional, _)) in default.iter() {
            if let Some(ref opt) = optional {
                // We'll take care of optional dependencies later.
                write!(w, "\n{}  {}", indent, opt)?;
            }
        }
        ...
        for (_, dep) in meta.dependencies.iter()
            .chain(meta.build_dependencies.iter())
            .chain(meta.target_dependencies.iter().flat_map(|&(_, ref y)| y.iter()))
            .filter(|&(_, c)| c.cr.found_in_lock && !c.cr.name.is_empty()) {

                at_least_one = true;
                write!(
                    w,
                    "\n{}  (if deps.\"{}\".\"{}\" ? \"{}\" then {}features_.{}.\"${{deps.\"{}\".\"{}\".\"{}\" or \"\"}}\" deps else {{}})",
                    indent,
    ...
    pub fn output_package<W: Write>(
        ...
        match meta.src {
            ...
            Src::Path {
                ...
                let workspace_member = workspace_member.as_ref().map(|x| {
...
fn output_current_prefix<W:Write>(w: &mut W, indent: &str, current_prefix: &mut &str, current_attrs: &mut Vec<String>) -> Result<(), Error> {
    if !current_prefix.is_empty() {
        ...
        if current_attrs.len() == 1 {
            writeln!(
                ...
                "{}  {}.{}",
                indent,
                current_prefix.trim_end_matches('.'),
                current_attrs.pop().unwrap()
            )?;
...
fn output_self_features<W:Write>(w: &mut W, indent: &str, output_features: &BTreeMap<String, Vec<String>>, full_name_default: &str) -> Result<(), Error> {
    ...
    for (a, b) in output_features.iter() {
        if !b.is_empty() {
            if current_prefix.is_empty() || !a.starts_with(current_prefix) {
                ...
                output_current_prefix(w, &indent, &mut current_prefix, &mut current_attrs)?;
                // open new prefix
        ...
        if b.len() == 0 {
            ...
            if b[0] == "true" || a == &full_name_default {
                current_attrs.push(format!(
                    "{} = {};",
                    a.trim_start_matches(current_prefix),
                    b[0]
                ))
            } else if b[0] != "false" {
                current_attrs.push(format!(
